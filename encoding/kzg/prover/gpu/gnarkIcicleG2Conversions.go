...
func GnarkG2AffineToIcicleG2Affine(as []bn254.G2Affine) []icicle_g2.G2Affine {
...
func GnarkG2AffineToProjective(as []bn254.G2Affine) []icicle_g2.G2Projective {
	fields := make([]fp.Element, 4*len(as))
	for i := 0; i < len(as); i++ {
		a := as[i]
		fields[i*4].Set(&a.X.A0)
	...
	projectives := make([]icicle_g2.G2Projective, len(affines))
...
func ConvertFpG2ToAffinesBytes(fpElements []fp.Element) []icicle_g2.G2Affine {
	baseFields := make([]icicle_g2.G2BaseField, len(fpElements))

	affines := make([]icicle_g2.G2Affine, len(baseFields)/4)
...
func ProjectiveToGnarkG2Affine(p icicle_g2.G2Projective) bn254.G2Affine {
